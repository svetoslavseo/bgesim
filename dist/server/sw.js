...
self.addEventListener('fetch', (event) => {
  ...
  // Bypass all service worker handling for localhost/dev and Vite internal endpoints
  // This prevents interference with Vite's dev server and HMR during development.
  try {
    const isLocalhost = self.location.hostname === 'localhost' || self.location.hostname === '127.0.0.1';
    const isViteInternal = url.pathname.startsWith('/@') || url.pathname.includes('__vite') || url.pathname.startsWith('/@react-refresh');
    if (isLocalhost || isViteInternal) {
      return; // Let the request pass through to network without SW handling
    }
  } catch (_) {
    // no-op if location is not available in SW context
  }
  ...
  if (
    ...
    return;
      ...
      // Handle flag images from local storage with cache
    if (url.pathname.startsWith('/esim-data/flags/')) {
    event.respondWith(
      caches.match(request).then((cachedResponse) => {
        if (cachedResponse) {
          return cachedResponse;
